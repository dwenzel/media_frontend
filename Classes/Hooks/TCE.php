 ...
 * TCE hook handling
 * 
 * Taken from Fabien Udriot's media extension and adapted by Dirk Wenzel
 * @package     TYPO3
 * @subpackage  speciality
 * @author Fabien Udriot <fabien.udriot@ecodev.ch>
 * @license     http://www.gnu.org/copyleft/gpl.html
 */
class Tx_MediaFrontend_Hooks_TCE {
	 ...
	 * @var string
	 */
	protected $extKey = 'media_frontend';
...
//	}
	...
	public function processDatamap_postProcessFieldArray($status, $table, $id, &$fieldArray, $pObj) {
		...
		if ($table === $this->tableName && !empty($pObj->uploadedFileArray[$this->tableName]['_userfuncFile']['file']['name'])) {
			...
			if (TRUE && $status == 'update') {
				$assetRepository = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('Tx_MediaFrontend_Domain_Repository_AssetRepository');
			...
			$fieldArray['sha1'] = $file->getSha1();
				...
				// Init service
			$metadataService = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('Tx_MediaFrontend_Service_Metadata');
			...
			if ($status == 'new') {
				$thumbnailService = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('Tx_MediaFrontend_Service_Thumbnail');
	...
	/**
	 * Check whether the BE User has acces to the field.
	 *
